...
1. Start quiz-appen ved å be Claude bruke `run-app` fra [oppgave 3](./03-lag-skill.md) for å kjøre opp appen, eller følg [`APPLICATION.md`](../APPLICATION.md).
2. Be Claude åpne `http://localhost:3000` med Chrome DevTools MCP, ta et snapshot og beskrive det som faktisk vises.
3. Be Claude starte quizen, svare på det første spørsmålet, bekrefte at appen går videre til spørsmål 2 og ta et skjermbilde av den nye tilstanden.
