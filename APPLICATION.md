...
> **Workshop:** Gå til [workshopoversikten](README.md) for å velge oppgavesett for OpenCode eller Claude Code.
...
- [OpenCode](https://opencode.ai/) eller [Claude Code](https://code.claude.com/docs/en/quickstart)
- [Volta](https://volta.sh/)
- The current stable version of [Google Chrome](https://www.google.com/chrome/)
- For OpenCode: a model provider account, such as GitHub Copilot, OpenAI, Anthropic, or Google
- For Claude Code: a Claude Pro, Max, Team, or Enterprise subscription, a Claude Console account, or access through a supported cloud provider
...
# Install your chosen agent and Chrome
...
```bash
opencode --version # OpenCode participants
claude --version   # Claude Code participants
volta --version
node --version
npx --version
...
Claude Code participants must use version **2.1.242 or newer**. Run `claude update` for a native installation or `brew upgrade claude-code` for Homebrew if your installed version is older.

The project pins Node.js `22.22.2` and pnpm `11.25.0` in `frontend/package.json`. You do not need to install pnpm globally; Volta selects the pinned version when commands are run from `frontend/`. Chrome DevTools MCP is also not installed globally; `npx` downloads it when the chosen agent starts the configured server.

If you do not want to use Homebrew, use the official installer for your chosen agent instead:
...
Authenticate your chosen agent and install the project dependencies before the session:
...
oppgaver-opencode/       OpenCode workshop track
oppgaver-claude/         Claude Code workshop track
.opencode/               OpenCode-specific workshop setup
.claude/                 Claude Code-specific workshop setup
